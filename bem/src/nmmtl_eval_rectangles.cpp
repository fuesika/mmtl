...
int nmmtl_evaluate_rectangles(int cntr_seg,
  ...
  LINE_SEGMENTS_P new_segment     = NULL;
  LINE_SEGMENTS_P last_segment    = NULL;
  LINE_SEGMENTS_P leading_segment = NULL;

  //-- Initialization ----------------------------------------------------------
  //ensure failure if variables are used unexpectedly
  save_divisions[0] = -1;
  save_divisions[1] = -1;
  save_length[0]    = -1.;
  save_length[1]    = -1.;
  ...
  sqrt((new_segment->endx - new_segment->startx)*
      ...
      save_divisions[0] = (int)(.99 + cntr_seg * new_segment->length / perimeter);

      /* force 2 divisions per side, minimum */
      if (save_divisions[0] < 2)
        save_divisions[0] = 2;
  ...
  sqrt((new_segment->endx - new_segment->startx)*
      ...
      /* straight calculation of number of divisions */
      save_divisions[1] = (int)(.99 + cntr_seg * new_segment->length / perimeter);

      /* force 2 divisions per side, minimum */
      if (save_divisions[1] < 2)
        save_divisions[1] = 2;
